...
/obj/item/grenade/chem_grenade/examine(mob/user)
	...
	if(user.can_see_reagents())
		if(beakers.len)
			. += span_info("You scan the grenade and detect the following reagents:")
			for(var/obj/item/reagent_containers/cup/glass_beaker in beakers)
				for(var/datum/reagent/reagent in glass_beaker.reagents.reagent_list)
					. += span_info("[reagent.volume] units of [reagent.name] in the [glass_beaker.name].")
			if(beakers.len == 1)
				. += span_info("You detect no second beaker in the grenade.")
		else
			. += span_info("You scan the grenade, but detect nothing.")
	else if(stage != GRENADE_READY && beakers.len)
		if(beakers.len == 2 && beakers[1].name == beakers[2].name)
			. += span_info("You see two [beakers[1].name]s inside the grenade.")
		else
			for(var/obj/item/reagent_containers/cup/glass_beaker in beakers)
				. += span_info("You see a [glass_beaker.name] inside the grenade.")
