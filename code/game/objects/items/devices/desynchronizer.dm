...
/obj/item/desynchronizer/examine(mob/user)
	. = ..()
	if(world.time < next_use)
		. += span_alert("Time left to recharge: [DisplayTimeText(next_use - world.time)]")
	. += span_info("Alt-click to customize the duration. Current duration: [DisplayTimeText(duration)].")
	. += span_info("Can be used again to interrupt the effect early. The recharge time is the same as the time spent in desync.")
