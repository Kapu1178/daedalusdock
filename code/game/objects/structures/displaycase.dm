...
/obj/structure/displaycase/examine(mob/user)
	. = ..()
	if(alert)
		. += span_info("Hooked up with an anti-theft system.")
	if(showpiece)
		. += span_info("There's \a [showpiece] inside.")
	if(trophy_message)
		. += "The plaque reads:\n [trophy_message]"
...
/obj/structure/displaycase/forsale/examine(mob/user)
	. = ..()
	if(showpiece && !open)
		. += span_info("[showpiece] is for sale for [sale_price] marks.")
	if(broken)
		. += span_info("[src] is sparking and the hover field generator seems to be overloaded. Use a multitool to fix it.")
