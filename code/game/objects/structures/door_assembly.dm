...
/obj/structure/door_assembly/examine(mob/user)
	...
	switch(state)
		if(AIRLOCK_ASSEMBLY_NEEDS_WIRES)
			if(anchored)
				. += span_info("The anchoring bolts are <b>wrenched</b> in place, but the maintenance panel lacks <i>wiring</i>.")
			else
				. += span_info("The assembly is <b>welded together</b>, but the anchoring bolts are <i>unwrenched</i>.")
		if(AIRLOCK_ASSEMBLY_NEEDS_ELECTRONICS)
			. += span_info("The maintenance panel is <b>wired</b>, but the circuit slot is <i>empty</i>.")
		if(AIRLOCK_ASSEMBLY_NEEDS_SCREWDRIVER)
			. += span_info("The circuit is <b>connected loosely</b> to its slot, but the maintenance panel is <i>unscrewed and open</i>.")
	if(!mineral && !glass && !noglass)
		. += span_info("There is a small <i>paper</i> placard on the assembly[doorname]. There are <i>empty</i> slots for glass windows and mineral covers.")
	else if(!mineral && glass && !noglass)
		. += span_info("There is a small <i>paper</i> placard on the assembly[doorname]. There are <i>empty</i> slots for mineral covers.")
	else if(mineral && !glass && !noglass)
		. += span_info("There is a small <i>paper</i> placard on the assembly[doorname]. There are <i>empty</i> slots for glass windows.")
	else
		. += span_info("There is a small <i>paper</i> placard on the assembly[doorname].")
