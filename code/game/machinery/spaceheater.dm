...
/obj/machinery/space_heater/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		. += span_info("The status display reads: Temperature range at <b>[settable_temperature_range]°C</b>.<br>Heating power at <b>[siunit(heating_power, "W", 1)]</b>.<br>Power consumption at <b>[(efficiency*-0.0025)+150]%</b>.") //100%, 75%, 50%, 25%
		. += span_info("<b>Right-click</b> to toggle [on ? "off" : "on"].")
