...
/obj/machinery/door/firedoor/examine(mob/user)
	. = ..()
	if(!density)
		. += span_info("It is open, but could be <b>pried</b> closed.")

	else if(!welded)
		. += span_info("It is closed, but could be <b>pried</b> open.")
		. += span_info("Hold the firelock temporarily open by prying it with <i>left-click</i> and standing next to it.")
		. += span_info("Prying by <i>right-clicking</i> the firelock will open it permanently.")
		. += span_info("Deconstruction would require it to be <b>welded</b> shut.")

	else if(boltslocked)
		. += span_info("It is <i>welded</i> shut. The floor bolts have been locked by <b>screws</b>.")
	else
		. += span_info("The bolt locks have been <i>unscrewed</i>, but the bolts themselves are still <b>wrenched</b> to the floor.")
...
/obj/structure/firelock_frame/examine(mob/user)
	...
	switch(constructionStep)
		if(CONSTRUCTION_PANEL_OPEN)
			. += span_info("It is <i>unbolted</i> from the floor. The circuit could be removed with a <b>crowbar</b>.")
			if(!reinforced)
				. += span_info("It could be reinforced with plasteel.")
		if(CONSTRUCTION_NO_CIRCUIT)
			. += span_info("There are no <i>firelock electronics</i> in the frame. The frame could be <b>welded</b> apart .")
