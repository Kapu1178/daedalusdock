...
/turf/closed/wall/proc/deconstruction_hints(mob/user)
	if(hard_decon)
		switch(d_state)
			if(INTACT)
				return span_info("The outer <b>grille</b> is fully intact.")
			if(SUPPORT_LINES)
				return span_info("The outer <i>grille</i> has been cut, and the support lines are <b>screwed</b> securely to the outer cover.")
			if(COVER)
				return span_info("The support lines have been <i>unscrewed</i>, and the metal cover is <b>welded</b> firmly in place.")
			if(CUT_COVER)
				return span_info("The metal cover has been <i>sliced through</i>, and is <b>connected loosely</b> to the girder.")
			if(ANCHOR_BOLTS)
				return span_info("The outer cover has been <i>pried away</i>, and the bolts anchoring the support rods are <b>wrenched</b> in place.")
			if(SUPPORT_RODS)
				return span_info("The bolts anchoring the support rods have been <i>loosened</i>, but are still <b>welded</b> firmly to the girder.")
			if(SHEATH)
				return span_info("The support rods have been <i>sliced through</i>, and the outer sheath is <b>connected loosely</b> to the girder.")
	else
		return span_info("The outer plating is <b>welded</b> firmly in place.")
