...
/datum/component/armor_plate/proc/examine(datum/source, mob/user, list/examine_list)
	...
	if(ismecha(parent))
		if(amount)
			if(amount < maxamount)
				examine_list += span_info("Its armor is enhanced with [amount] [upgrade_name].")
			else
				examine_list += span_info("It's wearing a fearsome carapace entirely composed of [upgrade_name] - its pilot must be an experienced monster hunter.")
		else
			examine_list += span_info("It has attachment points for strapping monster hide on for added protection.")
	else
		if(amount)
			examine_list += span_info("It has been strengthened with [amount]/[maxamount] [upgrade_name].")
		else
			examine_list += span_info("It can be strengthened with up to [maxamount] [upgrade_name].")
