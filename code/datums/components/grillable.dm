...
/datum/component/grillable/proc/OnExamine(atom/A, mob/user, list/examine_list)
	...
	if(!current_cook_time) //Not grilled yet
		if(positive_result)
			examine_list += span_info("[parent] can be <b>grilled</b> into \a [initial(cook_result.name)].")
		return

	if(positive_result)
		if(current_cook_time <= required_cook_time * 0.75)
			examine_list += span_info("[parent] probably needs to be cooked a bit longer!")
		else if(current_cook_time <= required_cook_time)
			examine_list += span_info("[parent] seems to be almost finished cooking!")
	else
