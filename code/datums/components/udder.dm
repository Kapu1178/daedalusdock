...
/datum/component/udder/proc/on_examine(datum/source, mob/user, list/examine_list)
	...
	switch(udder_filled_percentage)
		if(0 to 10)
			examine_list += span_info("[parent]'s [udder] is dry.")
		if(11 to 99)
			examine_list += span_info("[parent]'s [udder] can be milked if you have something to contain it.")
		if(100)
			examine_list += span_info("[parent]'s [udder] is round and full, and can be milked if you have something to contain it.")
