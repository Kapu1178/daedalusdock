...
/datum/blobstrain/debris_devourer/examine(mob/user)
	...
	var/obj/structure/blob/special/core/core = overmind.blob_core
	if (isobserver(user))
		. += span_info("Absorbed debris is currently reducing incoming damage by [round(max(min(DEBRIS_DENSITY, 10),0))]")
	else
		switch (round(max(min(DEBRIS_DENSITY, 10),0)))
			if (0)
				. += span_info("There is not currently enough absorbed debris to reduce damage.")
			if (1 to 3)
				. += span_info("Absorbed debris is currently reducing incoming damage by a very low amount.") // these roughly correspond with force description strings
			if (4 to 7)
				. += span_info("Absorbed debris is currently reducing incoming damage by a low amount.")
			if (8 to 10)
				. += span_info("Absorbed debris is currently reducing incoming damage by a medium amount.")
