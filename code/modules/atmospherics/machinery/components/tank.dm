...
/obj/machinery/atmospherics/components/tank/examine(mob/user, thats)
	...
	if(!initialize_directions)
		. += span_info("A pipe port can be opened with a [wrench_hint].")
	else
		. += span_info("The pipe port can be moved or closed with a [wrench_hint].")
	. += span_info("A holographic sticker on it says that its maximum safe pressure is: [siunit_pressure(max_pressure, 0)].")
...
/obj/structure/tank_frame/examine(mob/user)
	...
	if(!anchored)
		. += span_info("[src] has not been [wrenched_hint] to the floor yet.")
	else
		. += span_info("[src] is [wrenched_hint] to the floor.")

	switch(construction_state)
		if(TANK_FRAME)
			var/screwed_hint = EXAMINE_HINT("screwed")
			var/plating_hint = EXAMINE_HINT("metal plating")
			. += span_info("[src] is [screwed_hint] together and now just needs some [plating_hint].")
		if(TANK_PLATING_UNSECURED)
			var/crowbar_hint = EXAMINE_HINT("crowbar")
			var/welder_hint = EXAMINE_HINT("welder")
			. += span_info("The plating has been firmly attached and would need a [crowbar_hint] to detach, but still needs to be sealed by a [welder_hint].")
