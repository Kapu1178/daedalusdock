...
/datum/action/vehicle/sealed/mecha/ivanov_strike/proc/start_missile_targeting()
	...
	SEND_SOUND(owner, sound('sound/machines/terminal_on.ogg', channel = SSsounds.random_available_channel())) //spammable so I don't want to make it audible to anyone else
...
/datum/action/vehicle/sealed/mecha/ivanov_strike/proc/drop_missile(turf/target_turf)
	...
	if(rockets_left <= 0)
		end_missile_targeting()
	SEND_SOUND(owner, sound('sound/machines/triple_beep.ogg', channel = SSsounds.random_available_channel()))
